...
def clean_file(text):
    ...
    f2 = open('invariante.txt.tmp', 'w')
    for line in f1:
        f2.write(line.replace('The net is covered by positive T-Invariants, therefore it might be bounded and live.', ''))
    f1.close()
    f2.close()
    ...
    f2 = open('invariante.txt', 'w')
    for line in f1:
        f2.write(line.replace('The net is covered by positive P-Invariants, therefore it is bounded.', ''))
    f1.close()
    f2.close()
    ...
    f2 = open('invariante.txt.tmp', 'w')
    for line in f1:
        f2.write(line.replace('P-Invariants', ''))
    f1.close()
    f2.close()
    ...
    f2 = open('invariante.txt', 'w')
    for line in f1:
        f2.write(line.replace('T-Invariants ', ''))
    f1.close()
    f2.close()
    ...
    f2 = open('invariante.txt.tmp', 'w')
    for line in f1:
        f2.write(line.replace('P-Invariants equations', ''))
    f1.close()
    f2.close()
...
def main():
    ...
    # para que no pida el path se lo pone por defecto siempre el mismo
    #archivo = input("Path del archivo de Invariantes (html): ")
    archivo = os.path.dirname(os.path.realpath(__file__))+"/tmp/inv.html"
    html = open(archivo,'r')
